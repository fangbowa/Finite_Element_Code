...
inc_strain=[ 0.00001*ones(1,500)...
            ...
            -0.00001*ones(1,1500) 0.00001*ones(1,2000) ...
            -0.00001*ones(1,1000)];
E=1e8; % unit:Pa
yield_tag=0;
strength=1e5;
...
for i=2:length(inc_strain)
    ...
    if yield_tag==0
        ...
        if L<0
           ...
           E_ep=E-E*E/(E+2/3*ha);
           stress(i)=stress(i-1)+E_ep*inc_strain(i); 
           inc_strain_pl=E/(E+2/3*ha)*inc_strain(i);
           total_strain_pl(i)=total_strain_pl(i-1) + inc_strain_pl;
           yield_tag=1;
           alpha(i)=alpha(i-1)+2/3*ha*inc_strain_pl;
           i
        end
...
title({'1D von-Mises hysteresis curve (Prager hardening) with',  'E=1e8 Pa, strength=1e5, c=2e7'})
